...
export async function runDoer(
  ...
  // CLI shims (and the Local LLM HTTP shim, which still consumes the
  // user's local GPU/RAM) go through the semaphore. Only true remote
  // HTTP shims (openrouter — hosted gateway) bypass it via
  // bypassesLocalCliSemaphore. The slot is held until the doer returns;
  // cross-lineage fallback within the slot doesn't refresh the slot
  // (conservative — see reviewer-driver for the same trade-off). The
  // abortSignal lets a cancelled chat unwind a queued doer without
  // blocking the semaphore head forever.
  ...
  if (
    !skipSemaphore &&
    (CLI_LINEAGES as readonly string[]).includes(agentName)
  ) {
    try {
      releaseSlot = await acquireCliSlot(
        agentName as CliLineageKey,
        abortSignal,
      );
    } catch {
  ...
  try {
      ...
      try {
        ...
        return await runWithChainFallback(
          ...
          async (entry) => {
            ...
            const entryShim =
              entry.lineage === phase.doer.lineage
                ? shim
    ...
    const sessionName = sanitizeName(
      `chorus-${chatId}-${phase.id}-doer-${agentName}`,
    );
    ...
    const pollHandle = setInterval(() => {
      try {
        ...
        const err = errorDetector.inspect(
          session.name,
          phase.doer.lineage,
          pane,
        );
        if (err) {
          const recoveryKeys =
            err.kind === "permission_prompt"
              ? shim.recoverKeys?.permission_prompt
              : undefined;
          if (recoveryKeys && recoveryKeys.length > 0) {
            ...
            recordHealth({
              ...
              console.error(
                `[chorus] recordHealth failed for ${phase.doer.lineage}:`,
                healthErr,
              );
            });
            onEvent({
