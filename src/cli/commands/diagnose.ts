...
interface DiagnoseSnapshot {
  ...
  install: {
    binPath: string;
    mode: "global-npm" | "dev-tsx" | "local-dist" | "unknown";
  };
  daemon: {
    daemonJson: string;
...
function filterBenignNoise(text: string): {
  ...
  // its closer.
  const orphanIdx = lines.findIndex((l) =>
    l.includes("code: 'UND_ERR_SOCKET'"),
  );
  let startIdx = 0;
  let orphanCount = 0;
  if (orphanIdx >= 0) {
 ...
 * `redactHomePaths()` so $HOME paths from spawn errors / process stderr
...
async function gather(): Promise<DiagnoseSnapshot> {
  ...
  // Smokes run in parallel via Promise.all so a 5-CLI fleet doesn't
  // block diagnose for 5×2s = 10s on the worst case.
  ...
  try {
    ...
    const smokes: Array<SmokeResult | undefined> = await Promise.all(
      found.map((d) =>
        d.found && d.path ? smokeOneCli(d.path) : Promise.resolve(undefined),
      ),
    );
    clis = found.map((d, i) => ({
      id: d.id,
  ...
  // auto-disable signal from the voice-failure-tracker so reporters
  // know when chorus has silently sidelined a model. Best-effort: same
  // DB connection as the chats/voices counts above.
  let voiceHealth: VoiceHealth = {
    total: 0,
    autoQuota: [],
    autoMissing: [],
    userDisabled: 0,
...
export function formatReport(s: DiagnoseSnapshot): string {
  ...
  lines.push(
    `running daemon:  ${s.chorus.runningDaemonVersion ?? "(not reachable)"}` +
      (s.chorus.mismatch
        ? "   ⚠ VERSION MISMATCH — run `chorus stop && chorus start`"
        : ""),
  );
  ...
  lines.push(
    `pid alive:       ${s.daemon.daemonPidAlive ?? "(no daemon.json)"}`,
  );
  lines.push(
  ...
  if (s.clis.length === 0) {
    ...
    for (const c of s.clis) {
      ...
      if (c.smoke) {
        if (c.smoke.ok) {
          lines.push(
            `      smoke: ok${c.smoke.version ? ` (v${c.smoke.version})` : ""}`,
          );
    ...
    for (const c of s.recentFailedChats) {
      ...
      if (c.erroredParticipants.length === 0) {
        ...
        for (const p of c.erroredParticipants) {
          ...
          lines.push(
            `        ${p.errorKind} (errorMessage: ${p.errorMessageBytes} bytes on disk)`,
          );
        }
      }
    }
