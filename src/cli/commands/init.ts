...
async function runOrchestratorAutoConnect(connectFlag?: string): Promise<void> {
  const { autoConnectAll } =
    await import("../../daemon/orchestrators/index.js");
...
export function registerInitCommand(program: Command): void {
  program
    ...
    .option(
      "--connect <list>",
      "Comma-separated list of CLIs to connect (claude,codex,gemini,opencode,kimi,grok,cursor,windsurf). Default: all detected.",
    )
    .action(async (opts: { register?: boolean; connect?: string }) => {
      try {
        ...
        if (detect.detectFailed) {
          ...
          console.log(
            c.dim(
              "    Chorus needs at least one of these (or an OpenRouter API key):",
            ),
          );
          console.log(
            c.dim(
          ...
          console.log(
            c.dim("      codex      — https://github.com/openai/codex"),
          );
          console.log(
            c.dim(
          ...
          console.log(
            c.dim("      kimi       — https://github.com/MoonshotAI/kimi-cli"),
          );
          console.log(c.dim("      grok       — https://x.ai/cli"));
          console.log(
            c.dim(
              "      openrouter — Settings → Voices → Add OpenRouter (uses your API key)",
            ),
          );
          console.log(
            c.dim(
              "    Install at least one CLI, or add an OpenRouter voice after `chorus start`.",
            ),
          );
        } else {
          console.log("");
