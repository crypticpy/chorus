/**
 * Regression test for upstream issue #49 — runner now honours all three
 ...
 * finding from upstream PR #50's chorus self-review: a doer that
 * crashed mid-stream must NOT be silently "accept-doer"'d.
 */

import { describe, it, expect } from "vitest";
import { decidePhaseOutcome } from "@/daemon/runner";

describe("decidePhaseOutcome (upstream issue #49)", () => {
  ...
  describe("doer crashed / no real disagreement (policy must NOT apply)", () => {
    // Convergent finding from upstream PR #50's chorus self-review: when
    // the doer crashed mid-stream (round loop exited via the
    // `!doerAnswer.full` break), accept-doer was silently accepting a
    // partial / empty answer as final. These cases pin the gate.
