...
describe("detectInstallMode", () => {
  ...
  it("classifies .ts source as dev-tsx", () => {
    expect(detectInstallMode("/home/dev/chorus/src/cli/index.ts")).toBe(
      "dev-tsx",
    );
  });

  it("classifies dist build as local-dist", () => {
    expect(detectInstallMode("/home/dev/chorus/dist/cli/index.js")).toBe(
      "local-dist",
    );
  });

  it("classifies windows-style dist path", () => {
    expect(detectInstallMode("C:\\proj\\chorus\\dist\\cli\\index.js")).toBe(
      "local-dist",
    );
  });
...
describe("abbreviateHome", () => {
  it("replaces home prefix with ~", () => {
    ...
    expect(abbreviateHome(`${home}/.chorus/daemon.log`)).toBe(
      "~/.chorus/daemon.log",
    );
  });
...
describe("formatReport", () => {
  it("renders a complete snapshot with version mismatch warning", () => {
    const out = formatReport({
      ...
      install: {
        binPath: "C:\\Users\\u\\AppData\\bin\\chorus.mjs",
        mode: "global-npm",
      },
      daemon: {
        daemonJson: '{\n  "daemonPort": 7707\n}',
  ...
  it("omits the mismatch warning when versions match", () => {
    const out = formatReport({
      ...
      install: {
        binPath: "/usr/lib/node_modules/chorus-codes/bin/chorus.mjs",
        mode: "global-npm",
      },
      ...
      clis: [
        {
          ...
          smoke: {
            ok: false,
            exitCode: 1,
            stderrFirstLine: "Error: not authenticated",
          },
        },
        {
          id: "codex-cli",
...
describe("readLatestAttempt", () => {
  it("returns errorKind + errorMessageBytes (NOT the raw message) from last JSONL row", () => {
    const tmp = path.join(
      os.tmpdir(),
      `chorus-attempts-${Date.now()}-${Math.random()}`,
    );
    fs.mkdirSync(tmp, { recursive: true });
    const file = path.join(tmp, "_attempts.jsonl");
    const longMessage =
      "Not authenticated — please run `opencode login` to refresh credentials";
    fs.writeFileSync(
      file,
      [
        JSON.stringify({
  ...
  it("returns null for empty file", () => {
    const tmp = path.join(
      os.tmpdir(),
      `chorus-attempts-empty-${Date.now()}-${Math.random()}`,
    );
    fs.mkdirSync(tmp, { recursive: true });
    const file = path.join(tmp, "_attempts.jsonl");
  ...
  it("skips malformed lines and returns the last valid one", () => {
    const tmp = path.join(
      os.tmpdir(),
      `chorus-attempts-bad-${Date.now()}-${Math.random()}`,
    );
    fs.mkdirSync(tmp, { recursive: true });
    const file = path.join(tmp, "_attempts.jsonl");
    fs.writeFileSync(
...
describe("resolveBinPath", () => {
  it("resolves a symlink to its real target", () => {
    ...
    const tmp = path.join(
      os.tmpdir(),
      `chorus-realpath-${Date.now()}-${Math.random()}`,
    );
    fs.mkdirSync(tmp, { recursive: true });
...
describe("detectInstallMode after realpath", () => {
  it("classifies a globally-installed bin as global-npm once symlink is followed", () => {
    ...
    const realLikePath = "/usr/lib/node_modules/chorus-codes/bin/chorus.mjs";
